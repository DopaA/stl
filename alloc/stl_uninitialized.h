...
ForwardIterator uninitialized_copy(InputIterator begin,InputIterator end,ForwardIterator be){
	return _unininialized_copy(begin,end,be,value_type(be));
}
...
//特化版本
ForwardIterator uninitialized_copy(const char*begin,const char*end,char *result){
	memove(result,begin,end-begin);
	return result+(end-begin);
}

ForwardIterator uninitialized_copy(const wchar_t*begin,const wchar_t*end,wchar_t*result){
	memove(result,begin,sizeof(wchar_t)*(end-begin));
	return result+(end-begin);
}
