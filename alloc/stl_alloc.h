...
__default_alloc_template::my_freelist[__LIST_COUNT]=
...
static void deallocate(void *p,size_t n){
	...
	obj *tmp=(obj*)p;
	tmp->freelist_link=*my_freelist;//my_freelist是链表中第一个的地址,也是数组的某个地址
	*my_freelist=tmp;
}
...
//用来重新填充
template<bool threads,int inst>
static void *__default_alloc_template::refill(size_t size){//此处的size已经ROUND_UP过
	int nobjs=20;//一次分配20个
	void* chunk=chunk_alloc(size,nobjs);
	if(1==nobjs){
		return chunk;
	}
	obj *volatile *freelist=my_freelist+FREELIST_INDEX(n);
...
//内存池管理
template<bool threads,int inst>
static char *__default_alloc_template::chunk_alloc(size_t n,int &nobjs){
	char *result;
	...
	if(bytes_left>bytes_toget){//如果完全满足
		...
		if(bytes_left>0){
			obj *volatile *freelist=free_list+FREELIST_INDEX(bytes_left);
		...
		if(start_free==0){
			...
			for(int i=size;i<=__MAX_BYTES;i+=__ALIGIN){
